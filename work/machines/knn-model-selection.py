...
def bias_variance_tradeoff():
    ...
    for k in neighbours:
        print 'KNeighborsRegressor(', k, ')'
        estimator = KNeighborsRegressor(k)
        mean_mse_train, std_mse_train, mean_mse_test, std_mse_test = kfold_for_regression(estimator, X, y, n_folds=8)
    ...
    plt.title("Bias-Variance Trade-Off for k-NN")
...
def learning_curve():
    ...
    folds = KFold(n=len(X), n_folds=10, shuffle=True)
    fig = plot_learning_curve(estimator, "50-NN learning curve", X, y, cv=folds, verbose=2, train_sizes=np.linspace(.1, 1.0, 50))
    fig.show()
